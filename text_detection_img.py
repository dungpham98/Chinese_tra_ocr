...
def process_image_folders(input_img_folder, output_folder):
    ...
    for fname in list_files:
        ...
        #np_img = np.ones((6000, 4000, 3), dtype=np.uint8) * 255  # background with white color
        ...
        for i in range(len(sort_array)):
            ...
            output = reader.readtext(output_file)
            if(output != []):
                if(output[0][1] != ''):
                    detect_count = detect_count + 1
                    out.append(output[0][1])
                    
                    #draw_text(np_img, output[0][1], (sort_array[i][0], sort_array[i][1]))
                else:
                    out.append("X")
            else:
                out.append("X")
